...
module MailCatcher
  module Mail
    ...
    def db
      @db ||= SQLite3::Database.new(":memory:", type_translation: true).tap do |db|
        db.execute(<<-SQL)
    ...
    def add_message(message)
      ...
      @add_message_query.execute(message[:sender], JSON.generate(message[:recipients]), mail.subject, message[:source],
      ...
      parts.each do |part|
        ...
        add_message_part(message_id, cid, part.mime_type || "text/plain", part.attachment? ? 1 : 0, part.filename,
                         part.charset, body, body.length)
      end
    ...
    def messages
      ...
      @messages_query.execute.map do |row|
        row.fields.zip(row).to_h.tap do |message|
    ...
    def message(id)
      ...
      row && row.fields.zip(row).to_h.tap do |message|
        message["recipients"] &&= JSON.parse(message["recipients"])
      end
    end
    ...
    def message_has_html?(id)
      ...
      !@message_has_html_query.execute(id).next.nil? || ["text/html",
                                                         "application/xhtml+xml"].include?(message(id)["type"])
    end
    ...
    def message_part_html(message_id)
      ...
      part || begin
        ...
        message if message && ["text/html", "application/xhtml+xml"].include?(message["type"])
      end
    end
